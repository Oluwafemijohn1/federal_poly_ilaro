...
public class SerialModule {
    ...
    private static final int FP_SetDri1 = 0x33;
    private static final int FP_SetDri2 = 0x34;
    private static final int FP_SetDri3 = 0x35;
    private static final int FP_SetDri4 = 0x36;
    private static final int FP_GetDri = 0x37;
                    ...
                    if (onIdentifyListener == null) {
                        ...
                        if (result != null) {
                            if (result[9] == 0x00) {
                                short pageId = getShort(result[10], result[11]);
                                short matchScore = getShort(result[12], result[13]);
                                onIdentifyListener.onIdentifySuccess(pageId, matchScore);
                                return;
                            }
                        }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case FP_GetImage:
                    ...
                    mMsgThreadHandler.obtainMessage(FP_GetImage, valueGetImage, -1).sendToTarget();
                    break;
                case FP_UpImage:
                    byte[] imageData = PSUpImage();
                    mMsgThreadHandler.obtainMessage(FP_UpImage, imageData).sendToTarget();
                    break;
                case FP_GenChar:
                    int valueGenChar = PSGenChar(msg.arg1);
                    mMsgThreadHandler.obtainMessage(FP_GenChar, valueGenChar, msg.arg1).sendToTarget();
                    break;
                case FP_RegModel:
                    int valueRegModel = PSRegModel();
                    mMsgThreadHandler.obtainMessage(FP_RegModel, valueRegModel, -1).sendToTarget();
                    break;
                case FP_UpChar:
                    byte[] charData = PSUpChar();
                    mMsgThreadHandler.obtainMessage(FP_UpChar, charData).sendToTarget();
                    break;
                case FP_DownChar:
                    int valueDownChar = PSDownChar((byte[]) msg.obj);
                    mMsgThreadHandler.obtainMessage(FP_DownChar, valueDownChar, -1).sendToTarget();
                    break;
                case FP_Match:
                    boolean valueMatch = PSMatch();
                    mMsgThreadHandler.obtainMessage(FP_Match, Boolean.valueOf(valueMatch)).sendToTarget();
                    break;
                case FP_StoreChar:
                    int valueStoreChar = PSStoreChar(msg.arg1, msg.arg2);
                    mMsgThreadHandler.obtainMessage(FP_StoreChar, valueStoreChar, -1).sendToTarget();
                    break;
                case FP_LoadChar:
                    int valueLoadChar = PSLoadChar(msg.arg1, msg.arg2);
                    mMsgThreadHandler.obtainMessage(FP_LoadChar, valueLoadChar, -1).sendToTarget();
                    break;
                case FP_Search:
                    byte[] result = PSSearch(msg.arg1, msg.arg2, (Integer) msg.obj);
                    mMsgThreadHandler.obtainMessage(FP_Search, result).sendToTarget();
                    break;
                case FP_DeleteChar:
                    int valueDeleteChar = PSDeleteChar((short) msg.arg1, (short) msg.arg2);
                    mMsgThreadHandler.obtainMessage(FP_DeleteChar, valueDeleteChar, -1).sendToTarget();
                    break;
                case FP_Empty:
                    int valueEmpty = PSEmpty();
                    mMsgThreadHandler.obtainMessage(FP_Empty, valueEmpty, -1).sendToTarget();
                    break;
                case FP_Enroll:
                    byte[] valueEnroll = PSEnroll();
                    mMsgThreadHandler.obtainMessage(FP_Enroll, valueEnroll).sendToTarget();
                    break;
                case FP_Identify:
                    byte[] valueIdentify = PSIdentify();
                    mMsgThreadHandler.obtainMessage(FP_Identify, valueIdentify).sendToTarget();
                    break;
                case FP_GetImageEX:
                    int valueGetImageEx = PSGetImageEx();
                    mMsgThreadHandler.obtainMessage(FP_GetImageEX, valueGetImageEx, -1).sendToTarget();
                    break;
                case FP_UpImageEX:
                    byte[] imageDataEx = PSUpImageEx();
                    mMsgThreadHandler.obtainMessage(FP_UpImageEX, imageDataEx).sendToTarget();
                    break;
                case FP_GenCharEX:
                    int valueGenCharEx = PSGenCharEx(msg.arg1);
                    mMsgThreadHandler.obtainMessage(FP_GenCharEX, valueGenCharEx, msg.arg1).sendToTarget();
                    break;
                default:
                    break;
            }
    ...
    public void FP_SetDri1() {
        SystemClock.sleep(100);
        mWorkerThreadHandler.sendEmptyMessage(FP_SetDri1);
    }

    public void FP_SetDri2() {
        SystemClock.sleep(100);
        mWorkerThreadHandler.sendEmptyMessage(FP_SetDri2);
    }

    public void FP_SetDri3() {
        SystemClock.sleep(100);
        mWorkerThreadHandler.sendEmptyMessage(FP_SetDri3);
    }

    public void FP_SetDri4() {
        SystemClock.sleep(100);
        mWorkerThreadHandler.sendEmptyMessage(FP_SetDri4);
    }

    public void FP_GetDri() {
        SystemClock.sleep(100);
        mWorkerThreadHandler.sendEmptyMessage(FP_GetDri);
    }
    ...
    public void FP_GenChar(int bufferId) {
        SystemClock.sleep(50);
        mWorkerThreadHandler.obtainMessage(FP_GenChar, bufferId, -1).sendToTarget();
    }
    ...
    public void FP_StoreChar(int bufferId, int pageId) {
        mWorkerThreadHandler.obtainMessage(FP_StoreChar, bufferId, pageId).sendToTarget();
    }

    public void FP_LoadChar(int bufferId, int pageId) {
        mWorkerThreadHandler.obtainMessage(FP_LoadChar, bufferId, pageId).sendToTarget();
    }

    public void FP_Search(int bufferId, int startPageId, int pageNum) {
        mWorkerThreadHandler.obtainMessage(FP_Search, bufferId, startPageId, pageNum).sendToTarget();
    }

    public void FP_DeleteChar(int pageIDStart, int delNum) {
        mWorkerThreadHandler.obtainMessage(FP_DeleteChar, pageIDStart, delNum).sendToTarget();
    }
    ...
    public void FP_GenCharEx(int bufferId) {
        mWorkerThreadHandler.obtainMessage(FP_GenCharEX, bufferId, -1).sendToTarget();
    }

    private synchronized void PSSetDri1() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, 0x01, 0x00, 0x05, 0x42, 0x00, 0x00, 0x00, 0x48};
        Log.d("TAG", "PSSetDri1: " + "方向1");
        sendCommand(command);
        int length = read(buffer, 12, 660);
        printlog("PSSetDri1", length);
    }

    private synchronized void PSSetDri2() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, 0x01, 0x00, 0x05, 0x42, 0x01, 0x00, 0x00, 0x49};
        sendCommand(command);
        int length = read(buffer, 12, 660);
        printlog("PSSetDri2", length);
    }

    private synchronized void PSSetDri3() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, 0x01, 0x00, 0x05, 0x42, 0x02, 0x00, 0x00, 0x4A};
        sendCommand(command);
        int length = read(buffer, 12, 660);
        printlog("PSSetDri3", length);
    }

    private synchronized void PSSetDri4() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, 0x01, 0x00, 0x05, 0x42, 0x03, 0x00, 0x00, 0x4B};
        sendCommand(command);
        int length = read(buffer, 12, 660);
        printlog("PSSetDri4", length);
    }

    private synchronized void PSGetDri() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, 0x01, 0x00, 0x03, 0x43, 0x00, 0x47};
        sendCommand(command);
        int length = read(buffer, 12, 400);
        printlog("PSGetDri", length);
        if (length == 12) {
        }
    }

    private synchronized int PSGetImage() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, 0x01, 0x00, 0x03, 0x01, 0x00, 0x05};
        sendCommand(command);
        int length = read(buffer, 12, 200);
    ...
    private synchronized byte PSGenChar(int bufferId) {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x01, (byte) 0x00, (byte) 0x04,
                (byte) 0x02, (byte) bufferId, (byte) 0x00, (byte) (0x7 + bufferId)};
        sendCommand(command);
    ...
    private synchronized byte PSRegModel() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, 0x01, 0x00, 0x03, 0x05, 0x00, 0x09};
        sendCommand(command);
    ...
    private synchronized byte PSStoreChar(int bufferId, int pageId) {
        ...
        int checkSum = 0x01 + 0x00 + 0x06 + 0x06 + bufferId + (pageIDArray[0] & 0xff) + (pageIDArray[1] & 0xff);
        byte[] checkSumArray = short2byte((short) checkSum);
        // Log.i("whw",
        // "checkSumArray hex=" + DataUtils.toHexString(checkSumArray)
        // + "    checkSum=" + checkSum);
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x01, (byte) 0x00, (byte) 0x06, (byte) 0x06, (byte) bufferId, (byte) pageIDArray[0], (byte) pageIDArray[1], (byte) checkSumArray[0], (byte) checkSumArray[1]};
        sendCommand(command);
        int length = read(buffer, 12, 100);
    ...
    private synchronized byte PSLoadChar(int bufferId, int pageId) {
        byte[] pageIDArray = short2byte((short) pageId);
        int checkSum = 0x01 + 0x00 + 0x06 + 0x07 + bufferId + (pageIDArray[0] & 0xff) + (pageIDArray[1] & 0xff);
        byte[] checkSumArray = short2byte((short) checkSum);
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x01, (byte) 0x00, (byte) 0x06, (byte) 0x07, (byte) bufferId, (byte) pageIDArray[0], (byte) pageIDArray[1], (byte) checkSumArray[0], (byte) checkSumArray[1]};
        sendCommand(command);
        int length = read(buffer, 12, 100);
    ...
    private synchronized byte[] PSSearch(int bufferId, int startPageId, int pageNum) {
        byte[] startPageIDArray = short2byte((short) startPageId);
        byte[] pageNumArray = short2byte((short) pageNum);
        int checkSum = 0x01 + 0x00 + 0x08 + 0x04 + bufferId + (startPageIDArray[0] & 0xff) + (startPageIDArray[1] & 0xff) + (pageNumArray[0] & 0xff) + (pageNumArray[1] & 0xff);
        byte[] checkSumArray = short2byte((short) checkSum);
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x01, (byte) 0x00, (byte) 0x08, (byte) 0x04, (byte) bufferId, (byte) startPageIDArray[0], (byte) startPageIDArray[1], (byte) pageNumArray[0], (byte) pageNumArray[1], (byte) checkSumArray[0], (byte) checkSumArray[1]};
        sendCommand(command);
    ...
    private synchronized boolean PSMatch() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x01, (byte) 0x00, (byte) 0x03, (byte) 0x03, (byte) 0x00, (byte) 0x07};
        sendCommand(command);
    ...
    private synchronized byte[] PSEnroll() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x01, (byte) 0x00, (byte) 0x03, (byte) 0x10, (byte) 0x00, (byte) 0x14};
        sendCommand(command);
    ...
    private synchronized byte[] PSIdentify() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x01, (byte) 0x00, (byte) 0x03, (byte) 0x11, (byte) 0x00, (byte) 0x15};
        sendCommand(command);
    ...
    private synchronized byte PSDeleteChar(short pageIDStart, short delNum) {
        ...
        int checkSum = 0x01 + 0x07 + 0x0c + (pageIDArray[0] & 0xff) + (pageIDArray[1] & 0xff) + (delNumArray[0] & 0xff) + (delNumArray[1] & 0xff);
        byte[] checkSumArray = short2byte((short) checkSum);
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x01, (byte) 0x00, (byte) 0x07, (byte) 0x0c, pageIDArray[0], pageIDArray[1], delNumArray[0], delNumArray[1], checkSumArray[0], checkSumArray[1]};
        sendCommand(command);
        int length = read(buffer, 12, 100);
    ...
    private synchronized byte PSEmpty() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x01, (byte) 0x00, (byte) 0x03, (byte) 0x0d, (byte) 0x00, (byte) 0x11};
        sendCommand(command);
        int length = read(buffer, 12, 100);
    ...
    private synchronized byte[] PSUpChar() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x01, (byte) 0x00, (byte) 0x04, (byte) 0x08, (byte) 0x01, (byte) 0x00, (byte) 0x0e};
        sendCommand(command);
    ...
    private synchronized byte PSDownChar(byte[] model) {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x01, (byte) 0x00, (byte) 0x04, (byte) 0x09, (byte) 0x02, (byte) 0x00, (byte) 0x10};
        sendCommand(command);
        int length = read(buffer, 12, 100);
    ...
    private synchronized byte[] PSUpImage() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x01, (byte) 0x00, (byte) 0x03, (byte) 0x0a, (byte) 0x00, (byte) 0x0e};
        sendCommand(command);
    ...
    private synchronized int PSGetImageEx() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, 0x01, 0x00, 0x03, 0x30, 0x00, 0x34};
        sendCommand(command);
        int length = read(buffer, 12, 660);
    ...
    private synchronized byte PSGenCharEx(int bufferId) {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 01, (byte) 0x00, (byte) 0x04, (byte) 0x32, (byte) bufferId, (byte) 0x00, (byte) (0x37 + bufferId)};
        sendCommand(command);
        int length = read(buffer, 12, 500);
    ...
    private synchronized byte[] PSUpImageEx() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x01, (byte) 0x00, (byte) 0x03, (byte) 0x31, (byte) 0x00, (byte) 0x35};
        sendCommand(command);
    ...
    private void sendData(byte[] data) {
        byte[] dataPrefix = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x02, (byte) 0x00, (byte) 0x82};
        byte[] endPrefix = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0x08, (byte) 0x00, (byte) 0x82};
    ...
    private boolean checkCmdTag(byte[] data) {
        for (int i = 0; i < data.length - 4; ) {
            if (((byte) data[i] == (byte) (0xEF)) && ((byte) data[i + 1] == (byte) (0x01)) && ((byte) data[i + 2] == (byte) (0xFF)) && ((byte) data[i + 3] == (byte) (0xFF))) {
                return true;
            }
            break;
        }
    ...
    protected /*synchronized*/ int read(byte buffer[], int size, int waittime) {
        ...
        if (!mIsSPI) {
            ...
            for (int i = 0; i < mCurrentSize - 4; ) {
                ...
                if (((byte) mBuffer[i] == (byte) (0xEF)) && ((byte) mBuffer[i + 1] == (byte) (0x01)) && ((byte) mBuffer[i + 2] == (byte) (0xFF)) && ((byte) mBuffer[i + 3] == (byte) (0xFF))) {

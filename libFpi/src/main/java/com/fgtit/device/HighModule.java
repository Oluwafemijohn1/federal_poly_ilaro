...
public class HighModule  {
	...
	private synchronized byte[] PSUpChar() {
		...
		sendCommand(command);
		int length = readData(buffer, UP_CHAR_RESPONSE_SIZE,200);
		...
		if (length >= 256) {
			...
			System.arraycopy(buffer, 0, packets, 0, 256);
			return packets;			
		}
		*/
		return null;
	...
	private synchronized byte[] PSUpImage() {
		...
		sendCommand(command);
		int length = readImage(buffer, UP_IMAGE_RESPONSE_SIZE,100);
		Log.i("whw", "PSUpImage length=" + length);
		if (length >= UP_IMAGE_RESPONSE_SIZE) {
			byte[] packets = new byte[UP_IMAGE_RESPONSE_SIZE];
			System.arraycopy(buffer, 0, packets, 0, UP_IMAGE_RESPONSE_SIZE);
			return getFingerprintImage(packets);
		}
		return null;
    ...
    public boolean OpenDevice(String uartName, int baudRate) {
        if (!isOpen) {
            try {
                openSerialPort(uartName,baudRate);
                isOpen = true;
    ...
    public void openSerialPort(String uartName, int baudRate) throws SecurityException, IOException, InvalidParameterException {
        if (mSerialPort == null) {
    ...
    protected int readImage(byte buffer[],int size,int waittime) {
    	int time = 4000;
        int sleepTime = 50;
        ...
        if (mCurrentSize > 0) {
        	while (!shutDown) {
				...
				if (mCurrentSize>=UP_IMAGE_RESPONSE_SIZE) {
					shutDown = true;
				}
				tm++;
				if(tm>200)
    ...
    protected int readData(byte buffer[],int size,int waittime) {
    	int time = 4000;
        int sleepTime = 50;
        int length = time / sleepTime;
        boolean shutDown = false;
	...
	protected int read(byte buffer[],int size,int waittime) {
		int time = 4000;
		int sleepTime = 50;
		int length = time / sleepTime;
		boolean shutDown = false;
